...
async function* file(dir) {
  ...
  for (const nome of voci) {
    ...
    else if (ESTENSIONI.has(path.extname(nome))) yield completo;
  }
}
...
for await (const percorso of file(RADICE)) {
  ...
  // la documentazione può nominare i file di firma senza contenerli
  const soloDocumentazione = path.extname(percorso) === '.md';

  esaminati += 1;
  const contenuto = await readFile(percorso, 'utf8');
  for (const sospetto of SOSPETTI) {
    if (soloDocumentazione && sospetto.nome.startsWith('certificato')) continue;
