...
| 5 | Due browser distinti entrano nella stessa stanza | `tests/e2e/room-join.spec.ts` | ✅ |
| 6 | Le azioni si sincronizzano tramite server | `tests/e2e/sync.spec.ts` | ✅ |
| 7 | La riconnessione recupera lo stato | `tests/e2e/reconnect.spec.ts`, `apps/server/src/__tests__/reconnect.test.ts` | ✅ |
| 8 | La migrazione dell'host funziona | `apps/server/src/__tests__/host-migration.test.ts`, `tests/e2e/host-migration.spec.ts` | ✅ |
...
| 15 | Nessun placeholder o controllo inattivo | `pnpm lint` (regola custom `no-placeholder`) + `tests/e2e/no-dead-controls.spec.ts` | ✅ |
| 16 | Grafica coerente e originale | `pnpm generate:assets` produce tutto; nessuna URL remota (`tests/e2e/no-remote-assets.spec.ts`) | ✅ |
| 17 | Le schermate principali sono curate | `pnpm test:visual` (12 snapshot) | ✅ |
| 18 | Layout su viewport iPhone | `tests/visual` viewport 393×852 | ✅ |
| 19 | Safe area e tastiera gestite | `tests/visual/safe-area.spec.ts` | ✅ |
| 20 | Audio, musica e modalità silenziosa | `apps/web/src/audio/__tests__/mixer.test.ts` + E2E toggle | ✅ |
| 21 | La PWA è installabile | `tests/e2e/pwa.spec.ts` (manifest + SW + icone + offline) | ✅ |
| 22 | Wrapper iOS presente e sincronizzabile | `pnpm ios:sync --dry-run`, `apps/ios/` | ✅ |
| 23 | Provider locale iOS implementato | `apps/ios/plugin/` (Swift) + `packages/ai/src/providers/onDeviceApple.ts` + test isolati | ✅ |
| 24 | Le chiavi non sono esposte | `scripts/check-secrets.mjs` sul bundle | ✅ |
| 25 | Il server rifiuta azioni illegali | `apps/server/src/__tests__/illegal-actions.test.ts` (24 casi) | ✅ |
| 26 | Nessuna vulnerabilità critica nota | `SECURITY.md` §7 + `pnpm audit` | ✅ |
| 27 | Nessun errore in console durante una partita | `tests/e2e/console-clean.spec.ts` | ✅ |
| 28 | Rivincita senza duplicazioni né leak | `tests/e2e/rematch.spec.ts` + `apps/web/src/scene/__tests__/lifecycle.test.ts` | ✅ |
